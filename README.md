...
Verified with Paseo app, daemon and SDK **0.8.0 on macOS**. The manifest range is `>=0.8.0 <0.9.0`; future 0.8 releases still need verification. Native iOS/Android are untested; a compact browser preview is not an on-device test. [Host evidence and compatibility tests](docs/compatibility.md).
